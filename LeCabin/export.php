...
{   
    ...
    $sql = "SELECT invoice_id FROM sold GROUP BY invoice_id;";
    ...
    if ($stmt->rowCount() > 0)
    {
        ...
        $i = 0;
        $index = 0;
        for ($z = 0; $z < count($qtty_id); $z++)
        {
            recursive($index, $serv, $qtt, $id, $i, 0);
            $i++;
            $index++;
        }
        if ($who == "excel")
        {
            getService($conn, $array, "excel");
        }
        else
        {
	...
	if(isset($_POST["export"]))
	{
	  ...
	  $active_sheet->setCellValue('K1', 'Total I.G.I.C. Incluido');
    ...
    $i = 0;
	  foreach($result as $row)
	  {
		...
		$active_sheet->setCellValue('I' . $count, $row["total"] * 100 / 107);
		$active_sheet->getStyle('I' . $count)->getNumberFormat()->setFormatCode('#,##0.00 €');
		$active_sheet->setCellValue('J' . $count, $row["total"] * 100 / 107 * .07);
	  ...
	  $active_sheet->setCellValue('A' . ($count + 4), "Salón de Estética Le Cabín - N.I.F. ---------");
	    ...
	    <div id="mobile"></div>
    		...
    		<br>
              ...
              <div class="col-md-1" style="width:3%;"></div>
            		<div class="col-md-10">
                    <div class="row">
                <div class="col-md-7">Facturas: <?php echo " " . $date; ?>º Trimestre del año: <?php echo " " . $year . " Salón de Estética - Joana N.I.F. ---------";?></div>
                <div class="col-md-2">
                <form method="post">
