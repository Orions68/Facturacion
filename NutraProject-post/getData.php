...
function getService($conn, $services, $where)
{
    ...
    if (count($services) == count($services, COUNT_RECURSIVE)) 
    {
        ...
        {
            $sql = "SELECT product, price FROM product WHERE id='$services[$i]'";
    ...
    else
    {
        for ($i = 0; $i < count($services); $i++)
        {
            ...
            {
                $sql = "SELECT product, price FROM product WHERE id='" . $services[$i][$j] . "';";
                $stmt = $conn->prepare($sql);
                $stmt->execute();
                $row_service = $stmt->fetch(PDO::FETCH_OBJ);
                if ($where == "html")
                {
                    $service[$i][$j] = $row_service->product . "<br>";
                    $price[$i][$j] = $row_service->price . " €<br>";
                }
                else
                {
                    $service[$i][$j] = $row_service->product . "\n";
