    ...
    if ($ok) // Si $ok está a true, no hubo coincidencias.
    {
        ...
        if ($row->email != $email) // Verifico que el E-mail enviado por post es distinto que el de la tabla.
        {
            if ($row->phone != $phone)
            {
                if ($pass != "")
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', phone='$phone', email='$email', pass='$hash', bday='$bday' WHERE id=$id;"; // Preparo la consulta modificando todo.
                }
                else
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', phone='$phone', email='$email', bday='$bday' WHERE id=$id;"; // Preparo la consulta modificando todo.
                }
            }
            else
            {
                if ($pass != "")
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', email='$email', pass='$hash', bday='$bday' WHERE id=$id;"; // Preparo la consulta modificando todo.
                }
                else
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', email='$email', bday='$bday' WHERE id=$id;"; // Preparo la consulta modificando todo.
                }
            }
        }
        else // Si el E-mail está repetido, el cliente no cambió su E-mail.
        {
            if ($row->phone != $phone)
            {
                if ($pass != "")
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', phone='$phone', pass='$hash', bday='$bday' WHERE id='$id';"; // Preparo la consulta modificando todo.
                }
                else
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', phone='$phone', bday='$bday' WHERE id='$id';"; // Preparo la consulta modificando todo.
                }
            }
            else
            {
                if ($pass != "")
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', pass='$hash', bday='$bday' WHERE id='$id';"; // Preparo la consulta modificando todo menos los datos repetidos.
                }
                else
                {
                    $sql = "UPDATE client SET name='$name', surname='$surname', surname2='$surname2', address='$address', bday='$bday' WHERE id='$id';"; // Preparo la consulta modificando todo menos los datos repetidos.
                }
            }
        }
